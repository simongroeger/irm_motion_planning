...
def parse_args():
    ...
    parser.add_argument('--vis-final-joints', type=lambda x: (str(x).lower() == 'true'),    default=False)
    parser.add_argument('--vis-final-robot', type=lambda x: (str(x).lower() == 'true'),     default=True)
...
if args.vis_final_joints:
    ax0.plot(fin_movement[1,0], fin_movement[1,1], '-', c="blue", linewidth=2, label="joint 2")
    ax0.plot(fin_movement[2,0], fin_movement[2,1], '-', c="orange", linewidth=2, label="joint 3")
    ax0.plot(fin_movement[3,0], fin_movement[3,1], '-', c="darkgreen", linewidth=2, label="end effector")


if args.vis_final_robot:
    i = 0
    fr1, = ax0.plot(fin_movement[0:2,0,i], fin_movement[0:2,1,i], color="blue", linewidth=3, label="joint 2")
    fr2, = ax0.plot(fin_movement[1:3,0,i], fin_movement[1:3,1,i], color="orange", linewidth=3, label="joint 3")
    fr3, = ax0.plot(fin_movement[2:4,0,i], fin_movement[2:4,1,i], color="darkgreen", linewidth=3, label="end effector")
...
if args.animate:
    def update(frame):
        if args.vis_final_robot:
            fr1.set_xdata(fin_movement[0:2,0,frame])
            fr1.set_ydata(fin_movement[0:2,1,frame])
            fr2.set_xdata(fin_movement[1:3,0,frame])
            fr2.set_ydata(fin_movement[1:3,1,frame])
            fr3.set_xdata(fin_movement[2:4,0,frame])
            fr3.set_ydata(fin_movement[2:4,1,frame])

    ani = FuncAnimation(fig=fig, func=update, frames=N_timesteps, interval=50, repeat=False)
    
    ani.save('trajectory_robotmovement.gif', writer=PillowWriter(fps=20))
